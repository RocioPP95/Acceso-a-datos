		...
		try {
			...
			try {
				...
				throw new RuntimeException(
						"La búsqueda de series no es correcta. No hay ninguna serie Laura. Debería lanzar Excepcion");
			} catch (SerieNotFoundException e) {
				System.out.println(" >>>> Consulta correcta");
			...
			try {
				serie = service.consultarSerie(serie.getId());
				// La serie debería haber sido borrada y por tanto no existir. El servicio
				// debería lanzar un SerieNotFoundException
				throw new RuntimeException(
						"La consulta de serie no es correcta o el borrado no se ha hecho correctamente. ");
			} catch (SerieNotFoundException e) {
				System.out.println(" >>>> Consulta correcta");
			}
